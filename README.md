...
- **按类型归类**：spec / api / acceptance / tmp / reference / sql
- **AI 命名**：根据内容自动生成有意义的文件名
- **不限内容**：只管分类和命名，不限制文档内容和结构
- **周期性整合**：用户按需触发，由 AI 完成扫描、合并、归档

## 安装

```bash
# 推荐：通过 skills CLI 一键装入仓库下所有 skill
npx skills add zzusp/wow-doc

# 仅装某个 skill：--skill 指定 skill 名；-g 全局；-a 目标 agent；-y 自动确认
npx skills add zzusp/wow-doc --skill wow-doc -g -a claude-code -y

# 手动：将 skills/ 下的目录复制到本地 skill 目录
